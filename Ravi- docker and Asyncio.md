...
Docker commands can be executed by the root user or by providing sudo. We can run the Docker command with a normal user in the docker group.
~~~
...
Let's continue our journey with Docker by learning a bit more about Docker images. A Docker image is made up of filesystems layered over each other. At the
base is a boot filesystem, boots, which resembles the typical Linux/Unix boot
...
filesystem is unmounted to free up the RAM used by the initial disk image.
So far this looks pretty much like a typical Linux virtualization stack. Indeed,
Docker next layers a root filesystem, rootfs, on top of the boot filesystem. These
...
Inspiring_heyrovsky
~~~
The output above shows a single running container .this is the container you created earlier. The presence of your container in this output proves that it’s still running. you can also see that it was created 6 minutes ago and has been running for 6 minter
## Connecting to the Redis
 container Let's now update our Sinatra application to connect to Redis and store our incoming parameters. To do that, we're going to need to be able to talk to the Redis server. There are several ways we could do this; let's explore and see the pros and cons of each. The first method involves Docker's network stack. So far, we've seen Docker containers exposing ports and binding interfaces so that container services are exposed on the local Docker host's external network. In addition to this capability, Docker
...
Including 3rd party registries and secure on-premises registries, but Docker Hub is the default, and it’s the one we’ll use in this document.
~~~
...
Root@b8765d3a67a9:/#
~~~
If your shell prompt has changed again, you are back inside the container.
...
Think that this is a synchronous code because the second print is waiting 3 seconds to print “Ravi chandra” after “ravi chandra”. But this code is asynchronous.
## Coroutine
...
It makes the coroutine run concurrently, we should create tasks which is the third process.
...
Finished: 16:07:37
~~~
The above code is now running concurrently and the say_something() coroutine is no longer waiting for the say_something() coroutine to finish. It's rather a running the same coroutine with different parameters concurrently
example:
...
~~~
## Futures
A Future is a low-level available object that represents the result of an asynchronous computation. It is created by calling the asyncio—future () function.
~~~
...
async def main():
    ...
    except for asyncio.TimeoutError:
        print("Timed out!")
asyncio.run(main())
~~~
# Timed out!
...
The timeout here in the Future object is set to 1 second although the slow_operation() coroutine is taking 400 seconds to complete.
## conclusion
The function of asynchronous programming using the Python asyncio module. The asyncio gives us programmatic execution of when a block of code when we use the context switches. That means we can manage many complex methods that occur with concurred programming.
